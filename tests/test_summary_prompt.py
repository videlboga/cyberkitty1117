...
def test_default_prompt_has_new_instructions(monkeypatch):
    ...
    # Лимит по умолчанию 8
    assert 'от 3 до 8 ключевых тем' in cond
...
def test_topic_limit_override(monkeypatch):
    ...
    cond = captured['topics']
    assert 'от 3 до 5 ключевых тем' in cond
    assert 'от 3 до 8 ключевых тем' not in cond
...
def test_prompt_summary_override_replaces_default(monkeypatch):
    ...
    # Но лимит из condition_main всё равно применяется (он не часть prompt_summary)
    assert 'от 3 до 8 ключевых тем' in cond